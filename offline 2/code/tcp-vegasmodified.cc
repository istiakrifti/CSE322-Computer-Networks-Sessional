...
NS_LOG_COMPONENT_DEFINE("TcpVegasModified");
NS_OBJECT_ENSURE_REGISTERED(TcpVegasModified);

TypeId
TcpVegasModified::GetTypeId()
{
    static TypeId tid = TypeId("ns3::TcpVegasModified")
                            .SetParent<TcpNewReno>()
                            .AddConstructor<TcpVegasModified>()
                            .SetGroupName("Internet")
                            .AddAttribute("Alpha",
                                          "Lower bound of packets in network",
                                          UintegerValue(2),
                                          MakeUintegerAccessor(&TcpVegasModified::m_alpha),
                                          MakeUintegerChecker<uint32_t>())
                            .AddAttribute("Beta",
                                          "Upper bound of packets in network",
                                          UintegerValue(4),
                                          MakeUintegerAccessor(&TcpVegasModified::m_beta),
                                          MakeUintegerChecker<uint32_t>())
                            .AddAttribute("Gamma",
                                          "Limit on increase",
                                          UintegerValue(1),
                                          MakeUintegerAccessor(&TcpVegasModified::m_gamma),
                                          MakeUintegerChecker<uint32_t>());
    return tid;
}

TcpVegasModified::TcpVegasModified()
    : TcpNewReno(),
      m_alpha(2),
      m_beta(4),
      m_gamma(1),
...
TcpVegasModified::TcpVegasModified(const TcpVegasModified& sock)
    : TcpNewReno(sock),
      ...
      m_doingVegasNow(true),
      m_begSndNxt(0),
      m_prevThroughput(sock.m_prevThroughput)
{
    NS_LOG_FUNCTION(this);
}
...
TcpVegasModified::Fork()
{
    return CopyObject<TcpVegasModified>(this);
}

void
TcpVegasModified::PktsAcked(Ptr<TcpSocketState> tcb, uint32_t segmentsAcked, const Time& rtt)
{
...
void
TcpVegasModified::EnableVegas(Ptr<TcpSocketState> tcb)
{
...
void
TcpVegasModified::CongestionStateSet(Ptr<TcpSocketState> tcb, const TcpSocketState::TcpCongState_t newState)
{
...
void
TcpVegasModified::IncreaseWindow(Ptr<TcpSocketState> tcb, uint32_t segmentsAcked)
{
    ...
    { // A Vegas cycle has finished, we do Vegas cwnd adjustment every RTT.
        ...
        else
        {
            ...
            double currentThroughput = static_cast<double>(segCwnd) / m_minRtt.GetSeconds();
            ...
            { // Linear increase/decrease mode
                ...
                { 
                    if (currentThroughput > m_prevThroughput)
                    { 
                        segCwnd++;
                        m_alpha++;
                        m_beta++;
                ...
                {
                    if (m_alpha > 1 && currentThroughput > m_prevThroughput)
                    {
                        segCwnd++;
                    }
                    else if (m_alpha > 1 && currentThroughput < m_prevThroughput)
                    {
                        segCwnd--;
                        m_alpha--;
                        m_beta--;
            ...
            m_prevThroughput = currentThroughput;
...
std::string
TcpVegasModified::GetName() const
{
    return "TcpVegasModified";
}

uint32_t
TcpVegasModified::GetSsThresh(Ptr<const TcpSocketState> tcb, uint32_t bytesInFlight)
{
