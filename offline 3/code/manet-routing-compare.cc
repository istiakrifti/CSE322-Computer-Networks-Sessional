...
class RoutingExperiment
{
  ...
  private:
    ...
    int m_nSinks{20};                                      //!< Number of sink nodes.
...
RoutingExperiment::CommandSetup(int argc, char** argv)
{
    ...
    cmd.AddValue("packetsPerSecond", "Packets per second", pps); 
    cmd.AddValue("nodeSpeed", "Node speed in m/s", nSpeed); 
    cmd.Parse(argc, argv);
...
RoutingExperiment::Run()
{
    ...
    bool isNewFile = !std::ifstream(m_CSVfileName).good(); 
    ...
    int packetSize = 64;
    // std::string rate("2048bps");
    std::string rate = std::to_string(pps * packetSize * 8) + "bps";
    ...
    {
        totalPacketsSent += flow.second.txPackets;    // Use txPackets to get actual packets sent
        totalPacketsReceived += flow.second.rxPackets;
        totalPacketsDropped += flow.second.lostPackets;
        totalDelay += flow.second.delaySum.GetSeconds();
    }

    double packetDeliveryRatio = (double)totalPacketsReceived / totalPacketsSent * 100.0;
    double packetDropRatio = (double)totalPacketsDropped / totalPacketsSent * 100.0;
    double avgEndToEndDelay = totalDelay / totalPacketsReceived;
    ...
    out << nNodes << ","
        << pps << ","
        << nodeSpeed << ","
        << (totalPacketsReceived * packetSize * 8.0) / ((TotalTime-100.0) * 1000) << "," 
        << avgEndToEndDelay << ","
        << packetDeliveryRatio << ","
        << packetDropRatio
        << std::endl;
